...
func (l *SetUpUserConversationLogic) SetUpUserConversation(in *im.SetUpUserConversationReq) (*im.SetUpUserConversationResp, error) {
	...
	case constants.SingleChatType:
		...
		// 验证是否建立过会话
		conversationRes, err := l.svcCtx.ConversationModel.FindOneByConversationId(l.ctx, conversationId)
		if err != nil {
			// 建立会话
...
func (l *SetUpUserConversationLogic) setUpUserConversation(conversationId, userId, recvId string,
	...
	if err != nil {
		if err == immodels.ErrNotFound {
			...
			if err != nil {
				return errors.Wrapf(xerr.NewDBErr(), "ConversationsModel.Insert err %v, req %v", err, userId)
			}
		} else {
